...
```
One positive quality of this form is its innate ability to restrict sampling **\<commandGenerate\>** output to STDOUT. Streaming only  STDOUT to **assert_output_true** differienciates this form from the other one that always consumes STDOUT and STDERR. However, there are some downsides to thei piping approach: 
...
A piped version of **assert_output_{true/false}**.  This form is expained withing context of [**assert_output_true**](https://github.com/WhisperingChaos/assert.source.sh/blob/master/README.md#assert_output_true-commandexpected-arglist-inputdelim-commandgenerate-arglist).  
