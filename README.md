...
#### assert_continue
After invoking this function, the current process will continue execution through subsequent failures detected by an assertion.  When assertion failures occur in this mode, the fact that a failure occurred is remembered.  This is the default behavior of this assertion library.  Once testing completes, the **assert_return_code_set** can be called to establish the return code value for the entire test.  
